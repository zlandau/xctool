...
@interface OCTestSuiteEventStateTests : OCEventStateTests
...
- (void)testPublishFromStarted
{
  ...
  [self assertThat:state didPublish:endEvent];
...
- (void)testFinishTests
{
  ...
  NSArray *events = [self getPublishedEventsForStates:@[state,testAState,testBState]
                                      withBlock:^{
                                        [state publishEvents];
                                      }];
